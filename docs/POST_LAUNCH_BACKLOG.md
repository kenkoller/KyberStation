...
> **Re-prioritized 2026-05-15** after the V3.9-BT bench session revealed that KyberStation's emitted config.h has never been confirmed to boot on real Proffieboard hardware in production form. The Hardware Profiles MVP (#1 below) is the work item that determines whether KyberStation is a functional flash-to-saber tool or remains a design-only editor. Everything else on this list is polish in comparison and is demoted accordingly. See [`docs/research/HARDWARE_COMPATIBILITY_STRATEGY.md`](research/HARDWARE_COMPATIBILITY_STRATEGY.md) for the full plan.

1. **Hardware Profiles MVP (v0.17 priority — functionality, not polish).** New `packages/hardware-profiles/` data layer with `HardwareProfile` data shape + YAML manifests + codegen adapter + UI chassis picker + StatusBar chip + export-time guard. 5 seed profiles to start (stock Proffieboard V3, 89sabers V3.9, 89sabers V3.9-BT, Sabertrio BT Power Core, custom-paste). ~3-week sprint. Pre-work this week: stub the package as a no-op PR (~2h), email 89sabers for V3.9-BT factory config, focused codegen-correctness audit. Full plan in [`HARDWARE_COMPATIBILITY_STRATEGY.md`](research/HARDWARE_COMPATIBILITY_STRATEGY.md) §4–§5.
2. **Factory Config Import (v0.18 — the safety net for unprofiled chassis).** Companion to #1. User pastes their factory `config.h` once; KyberStation preserves their `CONFIG_TOP` + `BladeConfig` + prop defines verbatim and emits only the `presets[]` array. ~2-week sprint. Unblocks any vendor we haven't profiled yet.
3. **Cut `v0.21.1` patch tag.** 105 commits past `v0.20.3`. Includes Xenopixel V3 full board support, template-eval interpreter, Fredrik Style Editor Phases 1–7, 3D blade renderer + Hardware Preview, Fett263 Prop File Editor Level 1, mouse-driven swing sim, slow-mo, lexer hardening, sub-1024 polish. ~30 min sprint: draft `[Unreleased]→[0.21.1]` block in CHANGELOG, create the tag, push.
4. **Renderer-level golden-hash full coverage.** PR #307 shipped the card-snapshot + drawBlade harness; the blade-render pipeline still needs node-canvas-driven pixel-level coverage to safely take on `lib/blade/*` follow-on extractions or Visualizer Phase 2D post-processing.
5. **Wave 8 / Prop File Editor Level 2 — button routing sub-tab.** All 8 aux/gesture modulators already registered in `packages/engine/src/modulation/registry.ts`; `ModulatorPlateBar` already renders Button + Gesture categories. The remaining surface is the routing-sub-tab UI + binding shape extensions in `bladeStore`. Pre-shipped infrastructure makes this an L scope (~6–8h, splittable into 3 M-scope agent dispatches as proven in 2026-05-01 night session).
6. **Visualizer Phase 2C — 3D mouse interaction.** Phase 2A+2B shipped via PR #301. Phase 2C adds orbit-rotation from drag, swing-sim from drag velocity, click→clash, hold→lockup. Closes the "looks pretty but isn't interactive" gap. **Polish — demoted from #4 to #6 by the 2026-05-15 re-prioritization.**
7. **Crystal Vault panel + Re-attunement UI.** Long-standing design debt from `docs/KYBER_CRYSTAL_3D.md`. M-L scope; needs Ken's UX confirmation on entry-point shape (header button vs sidebar entry vs `/vault` route). **Polish — demoted from #5 to #7.**
8. **Mobile shell migration to Sidebar + MainContent.** Currently 4-tab swipe UI + `MergedDesignPanel`. Migration would unblock retiring `DesignPanel.tsx`, `DynamicsPanel.tsx`, `MergedDesignPanel`, and `uiStore.activeTab` together. Needs UX judgment on drawer vs bottom-sheet pattern at 375px. **Polish — demoted from #6 to #8.**
