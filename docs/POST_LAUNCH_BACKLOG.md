...
> **Re-prioritized 2026-05-15** after the V3.9-BT bench session revealed two things:
> 1. KyberStation's emitted `config.h` has never been confirmed to boot on real V3.9-BT hardware (4 hypotheses tested, all disconfirmed).
> 2. Modern vendor sabers (89sabers, Sabertrio with `SAVE_PRESET`, KR Sabers v3+) load preset data from an SD-card file at runtime, **not from compiled `config.h`** — so the firmware-flash path is the wrong workflow for the most common community hardware.
>
> The **ProffieOS Runtime Presets** path (#1 below) delivers the "design preset → put it on saber" pitch without needing vendor config knowledge or ST-Link debug. Hardware Profiles (now #2) remains relevant for the older DIY-build path. Everything else on this list is polish in comparison. See [`docs/research/PROFFIEOS_RUNTIME_PRESET_FORMAT.md`](research/PROFFIEOS_RUNTIME_PRESET_FORMAT.md) and [`docs/research/HARDWARE_COMPATIBILITY_STRATEGY.md`](research/HARDWARE_COMPATIBILITY_STRATEGY.md).

1. **ProffieOS Runtime Presets export — Phase A (v0.17 priority — primary workflow).** Emit `presets.ini` for modern vendor sabers with `SAVE_PRESET`. Phase A scope: reorder/rename/duplicate factory presets + reassign fonts. Codegen emitter + tests + format reference doc + CardWriter UI all shipped this PR. **Phase B** (color override via `builtin N M R,G,B ...` args) and **Phase C** (parameterized verbs like `standard`/`advanced` independent of factory bank) are deferred until per-chassis schema validation. See [`docs/research/PROFFIEOS_RUNTIME_PRESET_FORMAT.md`](research/PROFFIEOS_RUNTIME_PRESET_FORMAT.md).
2. **Hardware Profiles MVP (v0.17 priority — fallback path).** Now demoted from #1 because runtime presets sidestep the flash workflow for the 95% case. Hardware Profiles still matters for: (a) older DIY builds that don't ship `SAVE_PRESET`, (b) users who want to add new blade-style algorithms beyond what their factory firmware compiles in. Full plan in [`HARDWARE_COMPATIBILITY_STRATEGY.md`](research/HARDWARE_COMPATIBILITY_STRATEGY.md) §4–§5.
3. **Factory Config Import (v0.18 — the safety net for unprofiled chassis).** Companion to #2. User pastes their factory `config.h` once; KyberStation preserves their `CONFIG_TOP` + `BladeConfig` + prop defines verbatim and emits only the `presets[]` array. ~2-week sprint. Unblocks any vendor we haven't profiled yet.
4. **Cut `v0.21.1` patch tag.** 105 commits past `v0.20.3`. Includes Xenopixel V3 full board support, template-eval interpreter, Fredrik Style Editor Phases 1–7, 3D blade renderer + Hardware Preview, Fett263 Prop File Editor Level 1, mouse-driven swing sim, slow-mo, lexer hardening, sub-1024 polish. ~30 min sprint: draft `[Unreleased]→[0.21.1]` block in CHANGELOG, create the tag, push.
5. **Renderer-level golden-hash full coverage.** PR #307 shipped the card-snapshot + drawBlade harness; the blade-render pipeline still needs node-canvas-driven pixel-level coverage to safely take on `lib/blade/*` follow-on extractions or Visualizer Phase 2D post-processing.
6. **Wave 8 / Prop File Editor Level 2 — button routing sub-tab.** All 8 aux/gesture modulators already registered in `packages/engine/src/modulation/registry.ts`; `ModulatorPlateBar` already renders Button + Gesture categories. The remaining surface is the routing-sub-tab UI + binding shape extensions in `bladeStore`. Pre-shipped infrastructure makes this an L scope (~6–8h, splittable into 3 M-scope agent dispatches as proven in 2026-05-01 night session).
7. **Visualizer Phase 2C — 3D mouse interaction.** Phase 2A+2B shipped via PR #301. Phase 2C adds orbit-rotation from drag, swing-sim from drag velocity, click→clash, hold→lockup. Closes the "looks pretty but isn't interactive" gap. **Polish — demoted by the 2026-05-15 re-prioritization.**
8. **Crystal Vault panel + Re-attunement UI.** Long-standing design debt from `docs/KYBER_CRYSTAL_3D.md`. M-L scope; needs Ken's UX confirmation on entry-point shape (header button vs sidebar entry vs `/vault` route). **Polish.**
9. **Mobile shell migration to Sidebar + MainContent.** Currently 4-tab swipe UI + `MergedDesignPanel`. Migration would unblock retiring `DesignPanel.tsx`, `DynamicsPanel.tsx`, `MergedDesignPanel`, and `uiStore.activeTab` together. Needs UX judgment on drawer vs bottom-sheet pattern at 375px. **Polish.**
