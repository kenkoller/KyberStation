...
1. ~~**Cut `v0.21.1` patch tag.**~~ ✅ **Shipped 2026-05-12 (PR #314, tag `v0.21.1`).** Codename "Polyglot Release". 118 commits since `v0.20.3`. See CHANGELOG.md `[0.21.1]`.
2. **Renderer-level golden-hash full coverage.** PR #307 shipped the card-snapshot + `drawWorkbenchBlade` harness (9 cases). A post-release follow-up expanded `drawWorkbenchBlade` to **28 cases** (more styles, colors, state transitions, LED-count edge cases, shimmer extremes). The remaining gap is the inline-only code in `BladeCanvas.tsx`: ignition-flash radial burst (lines 1320–1331), motion-ghost swing trail (1210–1273), topology overlays for crossguard + triple fans (1537–1686), ambient mip-2 luma coupling (1346–1383). Extraction-then-test is the prerequisite for `lib/blade/*` follow-on work and Visualizer Phase 2D post-processing.
