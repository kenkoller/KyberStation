...
| 🟡 **Marketing site re-implementation** | (was PR #32, now closed-and-deferred) | L | **Mostly done.** Pages shipped via [PR #179](https://github.com/kenkoller/KyberStation/pull/179) (marketing-site-pages) on 2026-04-30 night: `/features`, `/showcase`, `/changelog`, `/faq` all exist. **🔴 Only remaining:** `/community` page (mentioned in original spec, not yet built). SEO infra (`robots.txt` / `sitemap.xml` / `siteConfig.ts`) shipped pre-launch in PR #69. Marketing site GIFs shipped via PR #184 (Sprint 3) + PR #185 (bloom fix). Active worktree `feat/marketing-site-expansion` may have more in flight. |
...
| **WebUSB FlashPanel — detect BFB2 + write to correct bank on dual-bank boards** | [`SESSION_2026-04-30_HARDWARE_RECOVERY.md`](SESSION_2026-04-30_HARDWARE_RECOVERY.md) | S–M | The flasher hardcodes write address to `STM32L4_FLASH_BASE = 0x08000000` (Bank 1). 89sabers V3.9 ships with `BFB2=1` (boot Bank 2), so flashing via WebUSB writes to a bank the chip isn't booting from — user sees "no change" not a brick, but it's confusing UX. Fix: have the flasher read OB during `connect()` (alt=1, address `0x1FFF7800`, 4 bytes for OPTR), decode bit 20 (BFB2), and either (a) write to the correct bank or (b) refuse with a clear "your board boots from Bank 2 but WebUSB writes Bank 1 — use dfu-util CLI per FLASH_GUIDE.md" message. Read-only OB access is safe and doesn't trigger the manifest-bug auto-reset that OB writes do. Touch points: `apps/web/lib/webusb/connect.ts` (read OB), `apps/web/lib/webusb/DfuSeFlasher.ts` (`flash()` start address). Driven by 2026-04-30 hardware recovery session findings. |
