...
| **Code-level TODOs sweep** | grep results | S each | ~~`WorkbenchLayout.tsx:669` (theme-row cap)~~ ✅ shipped via [PR #110](https://github.com/kenkoller/KyberStation/pull/110) · ~~`DeliveryRail.tsx:210` + `StatusBar.tsx:170,238` (global WebUSB store)~~ ✅ shipped via PR #104 · ~~`useClickToRoute.ts` (bladeStore-modulation-patch cleanup)~~ ✅ shipped via [PR #113](https://github.com/kenkoller/KyberStation/pull/113) on 2026-04-29 · ~~`packages/engine/src/modulation/sampler.ts` (v1.1 preon/ignition/retraction progress + `BladeConfig.clashDecay`)~~ ✅ shipped via [PR #114](https://github.com/kenkoller/KyberStation/pull/114) (clashDecay) + [PR #123](https://github.com/kenkoller/KyberStation/pull/123) (preon/ignition/retraction progress) on 2026-04-29 · ~~`BladeHardwarePanel.tsx` + `PowerDrawPanel.tsx` (left-rail-overhaul consumer migration)~~ ✅ files no longer exist on disk (verified 2026-05-01 afternoon — both `apps/web/components/editor/BladeHardwarePanel.tsx` and `PowerDrawPanel.tsx` deleted in earlier session, drift-bit cleared) · `GradientBuilder.tsx` ⛔ still active stub awaiting `GradientRegion` extraction from `ColorPanel.tsx`. **All sweep clusters except the GradientBuilder/GradientRegion extraction are closed.** |
...
| **Dead-code cleanup — `GradientBuilder.tsx` consumer-migration stub** | grep results | S | **Updated 2026-05-01 afternoon:** the original blocker (3 stubs all blocked on mobile migration) is now reduced to ONE remaining stub. `BladeHardwarePanel.tsx` + `PowerDrawPanel.tsx` were already deleted from the working tree; only `GradientBuilder.tsx` remains. **Why it's still here**: `GradientBuilder` is actively rendered by `ColorColumnB.tsx` (line 670) and `BladeStyleColumnB.tsx` (line 163). `ColorPanel.tsx`'s gradient region is a private `function GradientRegion()` (not exported). **Path forward**: extract `GradientRegion` to a shared `apps/web/lib/gradient/` module, update both `GradientBuilder` and `ColorPanel` to consume the shared module, then `GradientBuilder.tsx` becomes a thin re-export and can be inlined into its 2 consumers. Scope is small-to-medium; not blocked on anything else. |
