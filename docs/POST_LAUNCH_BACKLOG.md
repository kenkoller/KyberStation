...
> **Last audited: 2026-05-16** — re-prioritized after PR #325 (v0.22.0 Runtime Presets) merged + bench-validated on 89sabers V3.9-BT. Closed by recent merges since 2026-05-12 audit: Runtime Presets export Phase A + C with 16-bit RGB scaling fix (#325 — the smoking-gun saga, full case study in `docs/HARDWARE_FIDELITY_PRINCIPLE.md` audit history), saber profile migration fixtures (#327), Fett263 import round-trip fixtures (#328), codegen int bare-literal double-wrap fix (#329), Fett263 template registry argcount fix (#330), bench validation curated presets (#331), S1 audit batch (#332). Audit doc spawned: [`docs/research/EMIT_PARSER_AUDIT.md`](research/EMIT_PARSER_AUDIT.md) — the cheap-to-verify reference for every emit↔parser boundary.
>
> **Previous audits (2026-05-12, 2026-05-01 evening, 2026-04-30):** retained below in the historical session-recap sections.
...
5. **Hardware Profiles MVP (v0.17 priority — fallback path).** Demoted from #1 because runtime presets sidestep the flash workflow for the 95% case. Still matters for: (a) older DIY builds that don't ship `SAVE_PRESET`, (b) users who want to add new blade-style algorithms beyond what their factory firmware compiles in. Full plan in [`HARDWARE_COMPATIBILITY_STRATEGY.md`](research/HARDWARE_COMPATIBILITY_STRATEGY.md) §4–§5.
