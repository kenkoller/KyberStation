...
| P17 | ✅ | 2026-04-18 | Pause button click ✓ toggles "⏸Pause" ↔ "▶Paused". **FIXED P17-001:** undo/redo now revert state correctly after preset loads. Root cause was useHistoryTracking re-pushing the restored snapshot onto the past stack after undo, wiping the future stack and creating zombie entries. Fix: `historyRestoreFlag` module + guard in useHistoryTracking + begin/endHistoryRestore wrap in UndoRedoButtons. 4 new tests added. |
...
Legend: ⏳ pending · 🚧 in progress · ✅ passed · ⚠️ passed with notes · ⏭ deferred to human QA · ❌ blocked

---

## Follow-up wins shipped post-sweep (2026-04-18)

- **Keyboard shortcuts for the 7 newly-exposed effects** — R (Fragment) · V (Bifurcate) · G (GhostEcho) · P (Splinter) · E (Coronary) · X (GlitchMatrix) · H (Siphon). All one-shot (verified against engine effect classes). Observed (pre-existing, out of scope): `W` advertised for shockwave in ribbon/SettingsModal/docs but not wired in useKeyboardShortcuts.ts — flagged as small follow-up.
- **Modal a11y rollout completed** — useModalDialog now wired into AccessibilityPanel, PresetGallery Save, PresetBrowser Save, SaberProfileManager Copy-Presets, SettingsModal. All modals get ESC-to-close + Tab/Shift+Tab focus trap + data-autofocus initial focus + focus restoration. Original P32-001 follow-up list fully cleared.
