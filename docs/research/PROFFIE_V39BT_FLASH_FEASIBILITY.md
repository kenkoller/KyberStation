...
### 6. 2026-05-18 W2-prime experimental result — H4 (toolchain skew alone) is largely ruled out; H1 is now the leading hypothesis

Plan: [`/Users/KK/.claude/plans/you-re-picking-up-the-imperative-hinton.md`](file:///Users/KK/.claude/plans/you-re-picking-up-the-imperative-hinton.md).
Forensic artifacts: [`docs/research/89sabers-config-vs-handcrafted-diff-2026-05-18.txt`](89sabers-config-vs-handcrafted-diff-2026-05-18.txt), bench logs in [`scripts/hardware-test/bench-logs/2026-05-18/`](../../scripts/hardware-test/bench-logs/2026-05-18/).

**What we did.** Executed W2-prime exactly as described in the 2026-05-18 update §"Revised priorities" above: took a fresh backup of the bench board (4/4 SHAs matched May 14 baseline — `d881a8e7…`, `61d9f615…`, `4c2b2194…`, `5f4ecdb7…`), rehearsed full USB-only recovery via [`restore-factory.sh`](../../scripts/hardware-test/restore-factory.sh) (saber booted cleanly post-restore), compiled the 89Sabers factory source against the same `arduino-cli` 1.4.1 + `proffieboard:stm32l4` 4.6 toolchain we use for stock builds, then flashed via [`safe-flash.sh`](../../scripts/hardware-test/safe-flash.sh) to Bank 1 with `:leave`.

- **Toolchain**: `arduino-cli 1.4.1` (Homebrew, 2026-01-19 build), `proffieboard:stm32l4` core `4.6`.
- **FQBN**: `proffieboard:stm32l4:ProffieboardV3-L452RE:dosfs=sdmmc1,usb=cdc_msc`.
- **Source**: `~/ProffieOS/config/89sabers-factory-2026-05-18.h` (SHA `27a09dbd577cc13b0c0a7a4f59aa07bb14f721f98858b5b643c15cf4039c8736`) — verbatim copy of `"89sabers Config File/89sabers-config 25.h"` as sent by 89Sabers 2026-05-17/18.
- **Compile output**: `ProffieOS.ino.iap` = 219,216 bytes (43% of dual-bank 507,904 capacity; fits in a single Bank 1 write). Two linker warnings noted (`memory region SRAM2 not declared`; `start of section .bss changed by 8`) — pre-existing on stock builds, not specific to this config.
- **Flash**: `Erase` 100% (262,144 bytes), `Download` 100% (219,200 bytes after 16-byte DFU-suffix strip), `Submitting leave request`, `Transitioning to dfuMANIFEST state`. No `dfu-util` errors. Same flash flow as Gate 3's `restore-factory` rehearsal that immediately preceded it and which produced a booting saber.

**Result.** **Saber did not boot.** 30+ seconds of polling for either DFU re-enumeration (`0483:df11`) or USB-CDC enumeration (`/dev/cu.usbmodem*`) — neither appeared. The chip went completely silent: no USB enumeration of any kind, no LED activity reported by the operator. **Identical failure mode** to the 8 prior 2026-05 attempts (§"Today's failure modes" / "Prior session corroboration").

**Recovery.** Per the audit's "stop after one attempt without changing a variable" rule, did not reflash. Operator forced DFU mode (POWER+AUX + USB), [`restore-factory.sh`](../../scripts/hardware-test/restore-factory.sh) ran against `backups/89sabers-v39bt-factory-2026-05-14/`, saber booted cleanly to factory firmware (`v7.12, config/89sabers-config.h, prop: SaberFett263Buttons, buttons: 2, installed: Apr 21 2026 08:44:54`). USB-CDC enumeration restored. Full USB-only recovery from a post-failed-flash state is confirmed working end-to-end on the front-side-only chassis — the most important safety-net validation from this session, fully independent of the experiment outcome.

**What it means for H1 vs H4.**

H4 (toolchain skew + missing defines in our hand-crafted `89V3_allfont_BLE_OLED.h`) was the cheapest-to-test hypothesis: maybe the hand-crafted config was just wrong, and 89Sabers' real factory source would compile to something that boots. **The diff was modest** ([`89sabers-config-vs-handcrafted-diff-2026-05-18.txt`](89sabers-config-vs-handcrafted-diff-2026-05-18.txt), 304 lines, 117 unique to factory): mostly preset-list content and cosmetic trailing-whitespace cleanup. The structural deltas were small — handful of `FETT263_*` defines, `VOLUME 1800 → 1200`, `CLASH_THRESHOLD_G 4.5 → 2.5`, addition of `FETT263_DISABLE_CHANGE_FONT`, and placeholder BLE strings. Nothing that looked load-bearing for boot.

Since the byte-for-byte factory source compiled with our toolchain produces the *same* dead-saber result as our hand-crafted approximation, **H4 alone does not explain the failure.** Either (a) the toolchain divergence runs deeper than the config file (different ARM GCC version, ProffieOS branch, or build flags that 89Sabers uses internally vs upstream), or (b) the failure is not toolchain-driven at all and a vendor mechanism — H1's signature/loader gate, H2's coprocessor blob, or H3's stage-2 application chain — is real.

**H1 is now the leading hypothesis.** Bank 1's 256 KiB of dense vendor content with no ProffieOS strings, combined with the consistent silent-hang behavior across 9 attempts spanning multiple source variants and a factory-correct source, points to a Bank-1-resident component the chip's boot chain expects to find intact. Writing user firmware to `0x08000000` clobbers whatever it is, and the boot chain fails to recover. **Definitively resolving H1 still requires ST-Link** to single-step from reset and characterize the Bank 1 → Bank 2 handoff — which remains blocked on chassis access.

**Runtime preset path: fully validated, smoking-gun fix proven on real hardware.** After recovery, deployed a 4-preset test deck (factory Vader `builtin 2 1`, factory Luke `builtin 2 1`, KS Test Red 16-bit `advanced 65535,0,0…`, KS Test Green 16-bit `advanced 0,65535,0…`) via direct SD-card write with both `presets.ini` AND `presets.tmp` written byte-identical. All 4 loaded correctly via `list_presets`. The two KS test presets rendered pure red and pure green respectively — **first hardware confirmation that PR #325's 16-bit RGB encoding fix is producing correct visual output on the V3.9-BT**. Mixed-verb decks (`builtin` + `advanced` in the same `presets.ini`) work without issue.

**Incidental finding — factory original SD has a mis-indexed Vader entry.** The factory original SD (`backups/89sabers-sdcard-original/presets.ini` from board acquisition) lists Vader as `style=builtin 2 1`, but slot 2 in this firmware build is luke (the test deck observed Vader rendering as Cyan, matching luke's `Cyan,White` blade). Correct mapping per the factory config preset array is `builtin 1 1`. Cosmetic — does not affect any operational behavior — but worth knowing for anyone building decks that reference the factory style bank by index.

**Updated next-step priorities:**
- **W2.2 (ST-Link bench session) — even higher urgency, still blocked.** With H4-alone effectively ruled out, ST-Link is the only path to resolve H1 vs H2 vs H3. Pending the next chassis-open window.
- **NEW W2-prime-bis — try Bank 2 with custom-linked firmware** (rather than Bank 1, which W2-prime just covered). This requires building ProffieOS with a `0x08040000` link address (not the default arduino-cli output) and was not in scope today. Would discriminate between H1 (Bank 1 specifically required for vendor content) and H3 (Bank 2 is the boot source and Bank 1 is the application). Cheap to set up but real risk of brick if Bank 2 turns out to *also* be load-bearing — requires the same recovery rehearsal.
- **W2.3 (stock Proffieboard reference)** — moves up in priority. If a stock board flashes successfully with our exact toolchain (`arduino-cli 1.4.1` + `proffieboard:stm32l4 4.6`), that rules out *the toolchain itself* as a confound and strengthens the case that the V3.9-BT failure is vendor-specific rather than tools-specific.
- **W1.1 (preset coverage analysis)** — even more important now. With the runtime path validated end-to-end (including the 16-bit RGB fix), the question of "what % of the 455-preset gallery is expressible via runtime verbs vs flash-only" is the load-bearing product question. Continue as the highest-value zero-hardware task.
- **For users on V3.9-BT chassis: posture is unchanged and re-confirmed.** Do not attempt custom firmware flashing on the V3.9-BT. Use the runtime-preset path via SD card (writing both `presets.ini` and `presets.tmp`).

### Bottom line

The audit's core recommendation **stands and is now reinforced by experimental data**: runtime presets is the sanctioned path on the V3.9-BT; do not flash custom firmware on this chassis under current bench conditions. The 2026-05-18 W2-prime experiment took the cheapest discriminator the audit identified, ran it cleanly with a full recovery rehearsal first, and produced the predicted-failure-mode result — confirming that the problem is not just our hand-crafted config approximation. **H4 alone (toolchain skew at the config level) is now largely ruled out; H1 (Bank-1 vendor-loader gate) becomes the leading hypothesis**, and definitive resolution still requires ST-Link analysis blocked on chassis access. Updated plan: [`V39BT_FLASH_NEXT_STEPS.md`](V39BT_FLASH_NEXT_STEPS.md).
