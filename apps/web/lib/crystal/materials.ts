...
export function createBodyMaterial(opts: CrystalBodyMaterialOptions): CrystalBodyMaterial {
  ...
  const material = new THREE.MeshPhysicalMaterial({
    ...
    // flatShading intentionally omitted: geometry.ts encodes HYBRID
    // normals (faceted horizontally across segments, smooth vertically
    // across rings). flatShading:true would clobber them with per-pixel
    // dFdx/dFdy face normals, breaking the continuous highlight band.
...
export function createQrDecalMaterial(texture: THREE.Texture): THREE.MeshBasicMaterial {
  return new THREE.MeshBasicMaterial({
    ...
    opacity: 1.0,
    depthWrite: false,
    // Disable depth test so the QR always draws on top of the
    // transmissive body, guaranteeing phone cameras see clean modules
