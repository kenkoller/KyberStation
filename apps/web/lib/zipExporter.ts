...
WHAT THIS FILE CAN AND CAN'T DO (Phase A)
-----------------------------------------
CAN: reorder your saber's presets, rename them, duplicate entries,
assign different sound fonts and track files to each preset, and pick
which factory style is used per slot.

CANNOT (today): customize colors, blast/clash/lockup colors, ignition
or retraction timing. That requires either a firmware flash (the
"Download config.h" path KyberStation already supports) or a future
Phase B color-override feature.
...
export async function exportMultiPresetZip(options: MultiExportOptions): Promise<Blob> {
  const { presets, boardId, boardOptions, runtimeInstallTime, runtimeNumBlades } = options;
  const zip = new JSZip();
