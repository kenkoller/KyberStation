...
import {
  ...
  // per the multi-board field-coverage audit (B2, 2026-05-16). Before
  // this share, the live export path covered only 6 styles and 0
  // ignition fallbacks; 27 KyberStation styles silently degraded to
  // Steady (1) and 8 ignitions silently degraded to Standard (0).
  mapBladeEffect,
  mapIgnitionStyle,
  clampIgnitionSpeed,
...
/**
 * Map a KyberStation style ID to a Xenopixel V3 blade effect ID (0-7).
 *
 * Implementation note: delegates to `mapBladeEffect` from
 * `@kyberstation/codegen` (the `XenopixelEmitter` class's mapping
 * table). Until 2026-05-16 the live export path here used a narrower
 * 6-entry map; 27 KyberStation styles silently degraded to Steady (1)
 * with no degradation note. The shared map covers 16 additional styles
 * with thoughtful Xenopixel approximations and returns degradation
 * notes so the README can surface them to the user.
 */
function xenoStyleId(style: string): number {
  return mapBladeEffect(style)[0];
}

/**
 * Map a KyberStation ignition ID to a Xenopixel V3 ignition style ID
 * (0-11).
 *
 * Implementation note: delegates to `mapIgnitionStyle` from
 * `@kyberstation/codegen`. Until 2026-05-16 the live export path used
 * a 12-entry direct-match-only map; non-matching ignitions (KyberStation
 * `center`, `stutter`, `glitch`, `crackle`, `fracture`, `flashFill`,
 * `pulseWave`, `dripUp`) all silently fell back to Standard (0). The
 * shared map provides explicit per-ignition approximations.
 */
function xenoIgnitionId(ignition: string): number {
  return mapIgnitionStyle(ignition)[0];
}

/**
 * Compute the per-preset Xenopixel degradation notes for inclusion in
 * the ZIP's KYBERSTATION_README.txt. Returns an array of human-readable
 * strings — one per preset that had something approximated or clamped.
 */
function collectXenoDegradationNotes(presets: ExportPreset[]): string[] {
  const notes: string[] = [];
  for (let i = 0; i < presets.length; i++) {
    const preset = presets[i];
    const c = preset.config;

    const [, styleNote] = mapBladeEffect(c.style);
    if (styleNote) notes.push(`Preset ${i + 1} "${preset.name}": ${styleNote}`);

    const [, ignitionNote] = mapIgnitionStyle(c.ignition);
    if (ignitionNote) notes.push(`Preset ${i + 1} "${preset.name}": ${ignitionNote}`);

    const igRaw = c.ignitionMs ?? 200;
    const igClamped = clampIgnitionSpeed(igRaw);
    if (igClamped !== igRaw) {
      notes.push(
        `Preset ${i + 1} "${preset.name}": Ignition speed ${igRaw}ms clamped to ${igClamped}ms (Xenopixel V3 range ${XENO_IGNITION_SPEED_MIN}-${XENO_IGNITION_SPEED_MAX}ms)`,
      );
    }

    const reRaw = c.retractionMs ?? 500;
    const reClamped = clampRetractionSpeed(reRaw);
    if (reClamped !== reRaw) {
      notes.push(
        `Preset ${i + 1} "${preset.name}": Retraction speed ${reRaw}ms clamped to ${reClamped}ms (Xenopixel V3 range ${XENO_RETRACTION_SPEED_MIN}-${XENO_RETRACTION_SPEED_MAX}ms)`,
      );
    }
  }
  return notes;
 ...
 *
 * Speeds are clamped to the Xenopixel V3 firmware-supported ranges
 * (ignition 100-800ms, retraction 200-1000ms). Values outside those
 * ranges are silently truncated on-device, which can cause unpredictable
 * behavior — we clamp explicitly and surface a degradation note in the
 * README instead of pretending the request was honored.
 */
function generateXenoFontConfig(preset: ExportPreset, fontNumber: number): string {
  ...
  const ignitionSpeed = clampIgnitionSpeed(c.ignitionMs ?? 200);   // G — ignition speed ms
  const retractionSpeed = clampRetractionSpeed(c.retractionMs ?? 500); // H — retraction speed ms
...
export async function exportMultiPresetZip(options: MultiExportOptions): Promise<Blob> {
  ...
  switch (boardId) {
    ...
    case 'xenopixel': {
      ...
      // Surface degradation notes for any style/ignition that had to be
      // approximated, or any speed that had to be clamped, so the user
      // is not silently lied to about what their saber will show. Only
      // emitted when there's at least one note — silent-clean exports
      // stay quiet.
      const degradationNotes = collectXenoDegradationNotes(presets);
      if (degradationNotes.length > 0) {
        const noteFile = [
          'KyberStation → Xenopixel V3 export notes',
          '==========================================',
          '',
          'Some preset values did not have a direct Xenopixel V3 equivalent',
          'and were approximated, or were outside the firmware-supported speed',
          'range and were clamped. The list below summarizes every adjustment',
          'made when generating this SD card config.',
          '',
          ...degradationNotes.map((n) => `  - ${n}`),
          '',
          'Direct ProffieOS export preserves these settings without',
          'approximation if you have a Proffieboard V3 saber.',
          '',
        ].join('\n');
        zip.file('XENOPIXEL_NOTES.txt', noteFile);
      }
      break;
    }
  }
