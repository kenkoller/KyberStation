import type { Preset } from '../types.js';

export const EXTENDED_UNIVERSE_PRESETS: Preset[] = [
  // ── Cal Kestis Variants ──
  {
    id: 'eu-cal-kestis-green',
    name: 'Cal Kestis (Green)',
    character: 'Cal Kestis',
    era: 'expanded-universe',
    affiliation: 'jedi',
    tier: 'base',
    description:
      'Cal Kestis with a vivid green kyber crystal. A classic Jedi hue for the Fallen Order survivor.',
    hiltNotes: 'Customizable hilt rebuilt on Bogano, multiple material options.',
    config: {
      name: 'CalKestisGreen',
      baseColor: { r: 10, g: 225, b: 50 },
      clashColor: { r: 220, g: 255, b: 200 },
      lockupColor: { r: 160, g: 240, b: 140 },
      blastColor: { r: 130, g: 255, b: 100 },
      style: 'stable',
      ignition: 'spark',
      retraction: 'standard',
      ignitionMs: 300,
      retractionMs: 380,
      shimmer: 0.1,
      ledCount: 144,
    },
  },
  {
    id: 'eu-cal-kestis-orange',
    name: 'Cal Kestis (Orange)',
    character: 'Cal Kestis',
    era: 'expanded-universe',
    affiliation: 'jedi',
    tier: 'base',
    description:
      'Cal Kestis with a rare orange kyber crystal. A warm amber glow unique among Jedi.',
    config: {
      name: 'CalKestisOrange',
      baseColor: { r: 255, g: 140, b: 0 },
      clashColor: { r: 255, g: 230, b: 170 },
      lockupColor: { r: 255, g: 180, b: 80 },
      blastColor: { r: 255, g: 160, b: 40 },
      style: 'stable',
      ignition: 'spark',
      retraction: 'standard',
      ignitionMs: 300,
      retractionMs: 380,
      shimmer: 0.12,
      ledCount: 144,
    },
  },
  {
    id: 'eu-cal-kestis-purple',
    name: 'Cal Kestis (Purple)',
    character: 'Cal Kestis',
    era: 'expanded-universe',
    affiliation: 'jedi',
    tier: 'base',
    description:
      'Cal Kestis with a purple kyber crystal. A rich violet blade — one of the rarer player-selectable kyber colors in Fallen Order / Survivor.',
    config: {
      name: 'CalKestisPurple',
      baseColor: { r: 160, g: 30, b: 255 },
      clashColor: { r: 220, g: 180, b: 255 },
      lockupColor: { r: 190, g: 100, b: 255 },
      blastColor: { r: 180, g: 80, b: 255 },
      style: 'stable',
      ignition: 'spark',
      retraction: 'standard',
      ignitionMs: 300,
      retractionMs: 380,
      shimmer: 0.1,
      ledCount: 144,
    },
  },
  {
    id: 'eu-cal-kestis-red',
    name: 'Cal Kestis (Red - Dark Side)',
    character: 'Cal Kestis',
    era: 'expanded-universe',
    affiliation: 'sith',
    tier: 'detailed',
    description:
      'Cal Kestis in a dark-side alternate path. Bleeding red crystal with unstable, crackling energy.',
    config: {
      name: 'CalKestisRed',
      baseColor: { r: 255, g: 0, b: 10 },
      clashColor: { r: 255, g: 200, b: 160 },
      lockupColor: { r: 255, g: 100, b: 60 },
      blastColor: { r: 255, g: 70, b: 40 },
      dragColor: { r: 200, g: 0, b: 0 },
      style: 'unstable',
      ignition: 'glitch',
      retraction: 'shatter',
      ignitionMs: 250,
      retractionMs: 300,
      shimmer: 0.35,
      ledCount: 144,
      flicker: 0.6,
      noiseLevel: 0.15,
      swingFxIntensity: 0.5,
    },
  },

  // ── Cal Kestis (Survivor — Crossguard) ──
  {
    id: 'eu-cal-kestis-crossguard',
    name: 'Cal Kestis (Crossguard)',
    character: 'Cal Kestis',
    era: 'expanded-universe',
    affiliation: 'jedi',
    tier: 'detailed',
    screenAccurate: true,
    description:
      'Cal\'s crossguard stance from Jedi: Survivor. A heavier, more powerful fighting style with lateral energy vents.',
    hiltNotes: 'Rebuilt hilt with crossguard emitters, unlocked from High Republic tomb on Jedha.',
    topologyNotes: 'Main blade with two crossguard quillons.',
    config: {
      name: 'CalKestisCrossguard',
      baseColor: { r: 0, g: 160, b: 255 },
      clashColor: { r: 220, g: 240, b: 255 },
      lockupColor: { r: 160, g: 200, b: 255 },
      blastColor: { r: 120, g: 190, b: 255 },
      dragColor: { r: 255, g: 180, b: 0 },
      style: 'photon',
      ignition: 'spark',
      retraction: 'standard',
      ignitionMs: 400,
      retractionMs: 420,
      shimmer: 0.08,
      ledCount: 144,
      crossguard: true,
      quillonLength: 44,
      sparkSize: 0.6,
      swingFxIntensity: 0.4,
    },
  },

  // ── Cal Kestis (Survivor — Double-Bladed) ──
  {
    id: 'eu-cal-kestis-double',
    name: 'Cal Kestis (Double-Bladed)',
    character: 'Cal Kestis',
    era: 'expanded-universe',
    affiliation: 'jedi',
    tier: 'base',
    screenAccurate: true,
    description:
      'Cal\'s double-bladed configuration. Wide sweeping attacks with dual cyan blades extending from a staff hilt.',
    hiltNotes: 'Split hilt with dual emitters, can switch between single and double-bladed modes.',
    topologyNotes: 'Double-bladed staff saber.',
    config: {
      name: 'CalKestisDouble',
      baseColor: { r: 0, g: 160, b: 255 },
      clashColor: { r: 200, g: 230, b: 255 },
      lockupColor: { r: 140, g: 200, b: 255 },
      blastColor: { r: 100, g: 180, b: 255 },
      style: 'stable',
      ignition: 'center',
      retraction: 'center',
      ignitionMs: 280,
      retractionMs: 340,
      shimmer: 0.08,
      ledCount: 144,
    },
  },

  // ── Merrin ──
  {
    id: 'eu-merrin-green',
    name: 'Merrin (Green - Nightsister)',
    character: 'Merrin',
    era: 'expanded-universe',
    affiliation: 'neutral',
    tier: 'detailed',
    screenAccurate: false,
    description:
      'Merrin, the Nightsister ally of Cal Kestis. A speculative "what-if" saber tuned to her green magick aura — she does not canonically wield a lightsaber.',
    hiltNotes: 'Imagined lightsaber with Nightsister-influenced components (non-canonical).',
    config: {
      name: 'MerrinNightsister',
      baseColor: { r: 0, g: 210, b: 60 },
      clashColor: { r: 180, g: 255, b: 180 },
      lockupColor: { r: 100, g: 240, b: 120 },
      blastColor: { r: 80, g: 255, b: 100 },
      dragColor: { r: 0, g: 150, b: 80 },
      style: 'plasma',
      ignition: 'drip-up',
      retraction: 'unravel',
      ignitionMs: 340,
      retractionMs: 500,
      shimmer: 0.22,
      ledCount: 144,
      magickGlow: true,
      noiseLevel: 0.08,
      swingFxIntensity: 0.3,
    },
  },

  // ── Stellan Gios (High Republic) ──
  {
    id: 'eu-stellan-gios-blue',
    name: 'Stellan Gios (Blue - Crossguard)',
    character: 'Stellan Gios',
    era: 'expanded-universe',
    affiliation: 'jedi',
    tier: 'detailed',
    description:
      'Stellan Gios\'s blue crossguard lightsaber from the High Republic era. A noble, radiant azure blade.',
    hiltNotes: 'Ornate crossguard hilt of High Republic Jedi design, predating Kylo Ren\'s by centuries.',
    topologyNotes: 'Main blade with two crossguard quillons.',
    config: {
      name: 'StellanGios',
      baseColor: { r: 30, g: 120, b: 255 },
      clashColor: { r: 200, g: 220, b: 255 },
      lockupColor: { r: 140, g: 180, b: 255 },
      blastColor: { r: 100, g: 160, b: 255 },
      dragColor: { r: 255, g: 200, b: 0 },
      style: 'photon',
      ignition: 'scroll',
      retraction: 'scroll',
      ignitionMs: 380,
      retractionMs: 420,
      shimmer: 0.06,
      ledCount: 144,
      crossguard: true,
      swingFxIntensity: 0.25,
    },
  },

  // ── Vernestra Rwoh ──
  {
    id: 'eu-vernestra-rwoh-purple',
    name: 'Vernestra Rwoh (Purple - Lightwhip)',
    character: 'Vernestra Rwoh',
    era: 'expanded-universe',
    affiliation: 'jedi',
    tier: 'detailed',
    description:
      'Vernestra Rwoh\'s unique purple lightsaber that can convert into a lightwhip. A vibrant violet with fluid motion.',
    hiltNotes: 'Modified hilt with lightwhip conversion mechanism, youngest Jedi Knight in a generation.',
    topologyNotes: 'Standard blade mode, but hilt supports lightwhip extension.',
    config: {
      name: 'VernrestaRwoh',
      baseColor: { r: 140, g: 20, b: 255 },
      clashColor: { r: 210, g: 170, b: 255 },
      lockupColor: { r: 180, g: 100, b: 255 },
      blastColor: { r: 170, g: 80, b: 255 },
      style: 'pulse',
      ignition: 'scroll',
      retraction: 'scroll',
      ignitionMs: 350,
      retractionMs: 400,
      shimmer: 0.18,
      ledCount: 144,
      lightwhipMode: true,
      pulseSpeed: 1.0,
      pulseMinBright: 0.4,
      swingFxIntensity: 0.35,
    },
  },

  // ── Kelleran Beq ──
  {
    id: 'eu-kelleran-beq-blue',
    name: 'Kelleran Beq (Blue)',
    character: 'Kelleran Beq',
    era: 'expanded-universe',
    affiliation: 'jedi',
    tier: 'base',
    screenAccurate: true,
    description:
      'Kelleran Beq\'s blue lightsaber. The Sabered Hand, champion of the Jedi Temple training grounds who rescued Grogu during Order 66.',
    hiltNotes: 'Dual-wielder with standard Jedi hilts.',
    config: {
      name: 'KelleranBeq',
      baseColor: { r: 20, g: 90, b: 255 },
      clashColor: { r: 200, g: 220, b: 255 },
      lockupColor: { r: 150, g: 185, b: 255 },
      blastColor: { r: 110, g: 165, b: 255 },
      style: 'rotoscope',
      ignition: 'spark',
      retraction: 'standard',
      ignitionMs: 280,
      retractionMs: 350,
      shimmer: 0.08,
      ledCount: 144,
    },
  },

  // ── Shin Hati ──
  {
    id: 'eu-shin-hati-orange',
    name: 'Shin Hati (Orange)',
    character: 'Shin Hati',
    era: 'expanded-universe',
    affiliation: 'sith',
    tier: 'detailed',
    screenAccurate: true,
    description:
      'Shin Hati\'s red-orange lightsaber. Dave Filoni described the Ahsoka-series saber as intentionally fluctuating between red and orange, symbolic of her dark-side-coded moral ambiguity as Baylan Skoll\'s apprentice.',
    hiltNotes: 'Simple, functional hilt reflecting her pragmatic training under Baylan.',
    config: {
      name: 'ShinHati',
      baseColor: { r: 255, g: 100, b: 40 },
      clashColor: { r: 255, g: 225, b: 160 },
      lockupColor: { r: 255, g: 160, b: 60 },
      blastColor: { r: 255, g: 130, b: 30 },
      dragColor: { r: 200, g: 60, b: 0 },
      style: 'fire',
      ignition: 'scroll',
      retraction: 'scroll',
      ignitionMs: 300,
      retractionMs: 360,
      shimmer: 0.12,
      ledCount: 144,
      fireSize: 0.5,
      sparkRate: 0.6,
      heatSpread: 0.4,
      swingFxIntensity: 0.4,
    },
  },

  // ── Marrok ──
  {
    id: 'eu-marrok-red',
    name: 'Marrok (Red)',
    character: 'Marrok',
    era: 'expanded-universe',
    affiliation: 'sith',
    tier: 'detailed',
    screenAccurate: true,
    description:
      'Marrok\'s red lightsaber. A sinister, smoky crimson wielded by the mysterious dark-side mercenary. Upon death, green Nightsister magick was released.',
    hiltNotes: 'Inquisitor-style hilt beneath dark armor.',
    config: {
      name: 'Marrok',
      baseColor: { r: 230, g: 10, b: 10 },
      clashColor: { r: 255, g: 190, b: 140 },
      lockupColor: { r: 255, g: 90, b: 50 },
      blastColor: { r: 255, g: 70, b: 30 },
      dragColor: { r: 180, g: 0, b: 0 },
      style: 'fire',
      ignition: 'stutter',
      retraction: 'fadeout',
      ignitionMs: 260,
      retractionMs: 400,
      shimmer: 0.25,
      ledCount: 144,
      noiseLevel: 0.1,
      swingFxIntensity: 0.35,
    },
  },

  // ── Trilla Suduri (Fallen Order version) ──
  {
    id: 'eu-trilla-suduri-red',
    name: 'Trilla Suduri (Red)',
    character: 'Trilla Suduri',
    era: 'expanded-universe',
    affiliation: 'sith',
    tier: 'base',
    screenAccurate: true,
    description:
      'Trilla Suduri\'s red double-bladed lightsaber as the Second Sister in Jedi: Fallen Order. A controlled, lethal crimson.',
    hiltNotes: 'Inquisitor spinning double-bladed hilt.',
    config: {
      name: 'TrillaSuduri',
      baseColor: { r: 220, g: 5, b: 15 },
      clashColor: { r: 255, g: 185, b: 145 },
      lockupColor: { r: 255, g: 85, b: 60 },
      blastColor: { r: 255, g: 65, b: 40 },
      style: 'stable',
      ignition: 'center',
      retraction: 'center',
      ignitionMs: 220,
      retractionMs: 280,
      shimmer: 0.16,
      ledCount: 144,
    },
  },

  // ── Dagan Gera (Jedi: Survivor) ──
  {
    id: 'eu-dagan-gera-red',
    name: 'Dagan Gera (Red)',
    character: 'Dagan Gera',
    era: 'expanded-universe',
    affiliation: 'sith',
    tier: 'detailed',
    screenAccurate: true,
    description:
      'Dagan Gera\'s bled red lightsaber from Jedi: Survivor. Once a High Republic Jedi Knight, his crystal was bled (not cracked) into a stable red — his saber burns with betrayal and obsession, not instability.',
    hiltNotes: 'Ancient High Republic hilt design with split/crossguard capability.',
    topologyNotes: 'Convertible single/crossguard modes.',
    config: {
      name: 'DaganGera',
      baseColor: { r: 240, g: 15, b: 5 },
      clashColor: { r: 255, g: 210, b: 170 },
      lockupColor: { r: 255, g: 110, b: 70 },
      blastColor: { r: 255, g: 90, b: 40 },
      dragColor: { r: 200, g: 0, b: 0 },
      style: 'stable',
      ignition: 'standard',
      retraction: 'standard',
      ignitionMs: 260,
      retractionMs: 320,
      shimmer: 0.12,
      ledCount: 144,
      crossguard: true,
      swingFxIntensity: 0.35,
    },
  },

  // ── Bode Akuna (Jedi: Survivor) ──
  {
    id: 'eu-bode-akuna-red',
    name: 'Bode Akuna (Red)',
    character: 'Bode Akuna',
    era: 'expanded-universe',
    affiliation: 'sith',
    tier: 'detailed',
    screenAccurate: true,
    description:
      'Bode Akuna\'s revealed red lightsaber from Jedi: Survivor. A hidden Jedi turned ISB agent, his blade reflects his betrayal.',
    hiltNotes: 'Concealed standard hilt, dual-wielded with a blaster.',
    config: {
      name: 'BodeAkuna',
      baseColor: { r: 255, g: 20, b: 20 },
      clashColor: { r: 255, g: 200, b: 160 },
      lockupColor: { r: 255, g: 105, b: 65 },
      blastColor: { r: 255, g: 80, b: 45 },
      dragColor: { r: 200, g: 0, b: 0 },
      style: 'plasma',
      ignition: 'stutter',
      retraction: 'fadeout',
      ignitionMs: 280,
      retractionMs: 340,
      shimmer: 0.14,
      ledCount: 144,
      frequency: 1.2,
      phaseSpeed: 0.8,
      swingFxIntensity: 0.35,
    },
  },

  // ── Taron Malicos (Fallen Order) ──
  {
    id: 'eu-taron-malicos-red',
    name: 'Taron Malicos (Red)',
    character: 'Taron Malicos',
    era: 'expanded-universe',
    affiliation: 'sith',
    tier: 'detailed',
    screenAccurate: true,
    description:
      'Taron Malicos\'s dual red lightsabers from Dathomir. A fallen Jedi Master corrupted by the dark side and Nightbrother influence.',
    hiltNotes: 'Dual standard hilts, scarred and weathered from years on Dathomir.',
    config: {
      name: 'TaronMalicos',
      baseColor: { r: 210, g: 0, b: 5 },
      clashColor: { r: 255, g: 175, b: 130 },
      lockupColor: { r: 255, g: 80, b: 45 },
      blastColor: { r: 255, g: 55, b: 25 },
      dragColor: { r: 200, g: 0, b: 0 },
      style: 'fire',
      ignition: 'stutter',
      retraction: 'standard',
      ignitionMs: 240,
      retractionMs: 320,
      shimmer: 0.28,
      ledCount: 144,
      fireSize: 0.6,
      sparkRate: 0.5,
      heatSpread: 0.6,
      noiseLevel: 0.12,
      swingFxIntensity: 0.4,
    },
  },

  // ── Avar Kriss (High Republic) ──
  {
    id: 'eu-avar-kriss-green',
    name: 'Avar Kriss (Green)',
    character: 'Avar Kriss',
    era: 'expanded-universe',
    affiliation: 'jedi',
    tier: 'base',
    description:
      'Avar Kriss\'s green lightsaber from the High Republic. A brilliant, inspiring emerald blade befitting the Hero of Hetzal — she perceives the Force as a unifying "Song" connecting every Jedi.',
    hiltNotes: 'Elegant High Republic hilt, single green blade with a distinctive resonant hum consonant with her Song of the Force perception (per Light of the Jedi).',
    config: {
      name: 'AvarKriss',
      baseColor: { r: 0, g: 210, b: 45 },
      clashColor: { r: 200, g: 255, b: 190 },
      lockupColor: { r: 120, g: 240, b: 130 },
      blastColor: { r: 90, g: 255, b: 110 },
      style: 'stable',
      ignition: 'standard',
      retraction: 'standard',
      ignitionMs: 360,
      retractionMs: 400,
      shimmer: 0.07,
      ledCount: 144,
    },
  },
  {
    id: 'eu-keeve-trennis-green',
    name: 'Keeve Trennis',
    character: 'Keeve Trennis',
    era: 'expanded-universe',
    affiliation: 'jedi',
    tier: 'detailed',
    description: 'High Republic Jedi Knight with a green blade that shimmers like the aurora borealis.',
    config: {
      name: 'KeeveTrennis',
      baseColor: { r: 30, g: 200, b: 80 },
      clashColor: { r: 200, g: 255, b: 200 },
      lockupColor: { r: 150, g: 230, b: 150 },
      blastColor: { r: 255, g: 255, b: 255 },
      style: 'aurora',
      ignition: 'scroll',
      retraction: 'scroll',
      ignitionMs: 350,
      retractionMs: 400,
      shimmer: 0.08,
      ledCount: 144,
      waveCount: 3,
      driftSpeed: 0.5,
      swingFxIntensity: 0.3,
    },
  },
  {
    id: 'eu-loden-greatstorm-blue',
    name: 'Loden Greatstorm',
    character: 'Loden Greatstorm',
    era: 'expanded-universe',
    affiliation: 'jedi',
    tier: 'base',
    description: 'High Republic Jedi Master — a brilliant High Republic yellow blade radiating calm strength. (Canonically a yellow-bladed Jedi per Wookieepedia.)',
    config: {
      name: 'LodenGreatstorm',
      baseColor: { r: 255, g: 215, b: 0 },
      clashColor: { r: 255, g: 245, b: 200 },
      lockupColor: { r: 255, g: 225, b: 120 },
      blastColor: { r: 255, g: 255, b: 255 },
      style: 'photon',
      ignition: 'standard',
      retraction: 'standard',
      ignitionMs: 300,
      retractionMs: 400,
      shimmer: 0.05,
      ledCount: 144,
    },
  },
  {
    id: 'eu-marchion-ro-nihil',
    name: 'Marchion Ro (Nihil)',
    character: 'Marchion Ro',
    era: 'expanded-universe',
    affiliation: 'other',
    tier: 'detailed',
    description: 'The Eye of the Nihil — Marchion Ro wields Loden Greatstorm\'s looted yellow saber as a trophy; he has no personal lightsaber in canon. Presented here with the crackling unease of his Evereni touch on a once-proud Jedi blade.',
    hiltNotes: 'Loden Greatstorm\'s High Republic hilt, looted after Loden\'s death and carried as a trophy of the Nihil\'s war against the Jedi. Not Marchion Ro\'s own weapon.',
    config: {
      name: 'MarchionRo',
      baseColor: { r: 255, g: 215, b: 0 },
      clashColor: { r: 255, g: 245, b: 200 },
      lockupColor: { r: 255, g: 225, b: 120 },
      blastColor: { r: 255, g: 235, b: 160 },
      dragColor: { r: 200, g: 160, b: 0 },
      style: 'crystalShatter',
      ignition: 'glitch',
      retraction: 'shatter',
      ignitionMs: 400,
      retractionMs: 300,
      shimmer: 0.2,
      ledCount: 144,
      noiseLevel: 0.15,
      swingFxIntensity: 0.4,
    },
  },
  {
    id: 'eu-elzar-mann-purple',
    name: 'Elzar Mann',
    character: 'Elzar Mann',
    era: 'expanded-universe',
    affiliation: 'jedi',
    tier: 'detailed',
    description: 'Unconventional High Republic Jedi and Form V Guardian. Plasma arcs dance along his canonical blue blade, reflecting his emotional intensity.',
    config: {
      name: 'ElzarMann',
      baseColor: { r: 60, g: 80, b: 255 },
      clashColor: { r: 200, g: 210, b: 255 },
      lockupColor: { r: 140, g: 160, b: 255 },
      blastColor: { r: 255, g: 255, b: 255 },
      style: 'plasma',
      ignition: 'scroll',
      retraction: 'fadeout',
      ignitionMs: 350,
      retractionMs: 500,
      shimmer: 0.1,
      ledCount: 144,
      frequency: 1.0,
      phaseSpeed: 1.2,
      swingFxIntensity: 0.35,
    },
  },
  {
    id: 'eu-orla-jareni-white',
    name: 'Orla Jareni (Wayseeker)',
    character: 'Orla Jareni',
    era: 'expanded-universe',
    affiliation: 'jedi',
    tier: 'detailed',
    description: 'Independent Wayseeker who left the traditional Jedi path. White prismatic blade.',
    config: {
      name: 'OrlaJareni',
      baseColor: { r: 230, g: 235, b: 250 },
      clashColor: { r: 255, g: 255, b: 255 },
      lockupColor: { r: 200, g: 210, b: 240 },
      blastColor: { r: 255, g: 255, b: 255 },
      style: 'prism',
      ignition: 'center',
      retraction: 'center',
      ignitionMs: 300,
      retractionMs: 400,
      shimmer: 0.08,
      ledCount: 144,
      facets: 5,
      rotationSpeed: 1.0,
      swingFxIntensity: 0.2,
    },
  },
  {
    id: 'eu-burryaga-green',
    name: 'Burryaga (Wookiee Jedi)',
    character: 'Burryaga',
    era: 'expanded-universe',
    affiliation: 'jedi',
    tier: 'base',
    description: 'Rare Wookiee Jedi of the High Republic era. Oversized blue crossguard blade with slow, powerful ignition befitting his Wookiee stature.',
    hiltNotes: 'High Republic crossguard hilt sized for a Wookiee grip.',
    topologyNotes: 'Crossguard blade with lateral quillons.',
    config: {
      name: 'Burryaga',
      baseColor: { r: 0, g: 120, b: 255 },
      clashColor: { r: 200, g: 220, b: 255 },
      lockupColor: { r: 150, g: 185, b: 255 },
      blastColor: { r: 255, g: 255, b: 255 },
      style: 'stable',
      ignition: 'scroll',
      retraction: 'scroll',
      ignitionMs: 600,
      retractionMs: 500,
      shimmer: 0.05,
      ledCount: 144,
      crossguard: true,
    },
  },

  // ─── The Acolyte (~134 BBY, late High Republic era) ──────────────────
  //
  // Disney+ live-action 2024. Eight presets covering the saber-wielders
  // with meaningful screen time. Mae-ho Aniseya is intentionally omitted
  // because she fights with stolen / borrowed sabers throughout the show
  // and never has a defined personal blade. Osha Aniseya is omitted for
  // the same reason — her construction-arc saber lands at the very end
  // of the season with limited on-screen presence; revisit if a S2
  // expansion gives her a settled blade.

  // ── Sol (Acolyte) ──
  {
    id: 'eu-sol-green',
    name: 'Sol',
    character: 'Sol',
    era: 'expanded-universe',
    affiliation: 'jedi',
    tier: 'detailed',
    screenAccurate: true,
    description: 'Master Sol — calm, contemplative Jedi Master at the center of The Acolyte. A deep emerald-green blade befitting the Cerean-trained master who carries the weight of Brendok with him.',
    hiltNotes: 'Late-High-Republic single-emitter hilt with banded grip and leaf-engraved pommel.',
    config: {
      name: 'Sol',
      baseColor: { r: 30, g: 200, b: 60 },
      clashColor: { r: 220, g: 255, b: 220 },
      lockupColor: { r: 140, g: 240, b: 130 },
      blastColor: { r: 200, g: 255, b: 200 },
      style: 'stable',
      ignition: 'standard',
      retraction: 'standard',
      ignitionMs: 320,
      retractionMs: 380,
      shimmer: 0.06,
      ledCount: 144,
      swingFxIntensity: 0.3,
    },
  },

  // ── Indara (Acolyte) ──
  {
    id: 'eu-indara-blue',
    name: 'Indara',
    character: 'Indara',
    era: 'expanded-universe',
    affiliation: 'jedi',
    tier: 'base',
    screenAccurate: true,
    description: 'Master Indara — composed Jedi Master killed in The Acolyte\'s opening cantina duel. Steady, measured Jedi blue from a master who deflected fire mid-conversation.',
    hiltNotes: 'High Republic two-tone hilt with chrome accent ring.',
    config: {
      name: 'Indara',
      baseColor: { r: 30, g: 120, b: 255 },
      clashColor: { r: 220, g: 235, b: 255 },
      lockupColor: { r: 140, g: 200, b: 255 },
      blastColor: { r: 255, g: 255, b: 255 },
      style: 'stable',
      ignition: 'standard',
      retraction: 'standard',
      ignitionMs: 280,
      retractionMs: 340,
      shimmer: 0.08,
      ledCount: 144,
    },
  },

  // ── Yord Fandar (Acolyte) ──
  {
    id: 'eu-yord-fandar-blue',
    name: 'Yord Fandar',
    character: 'Yord Fandar',
    era: 'expanded-universe',
    affiliation: 'jedi',
    tier: 'base',
    screenAccurate: true,
    description: 'Yord Fandar\'s by-the-book Jedi Knight blade — bright cobalt blue from a young master who treats the Code as scaffolding. Killed by Mae in the Khofar attack.',
    hiltNotes: 'Standard High Republic Jedi Knight hilt, tightly machined.',
    config: {
      name: 'YordFandar',
      baseColor: { r: 60, g: 140, b: 255 },
      clashColor: { r: 230, g: 240, b: 255 },
      lockupColor: { r: 160, g: 210, b: 255 },
      blastColor: { r: 255, g: 255, b: 255 },
      style: 'stable',
      ignition: 'standard',
      retraction: 'standard',
      ignitionMs: 260,
      retractionMs: 320,
      shimmer: 0.08,
      ledCount: 144,
    },
  },

  // ── Jecki Lon (Acolyte) ──
  {
    id: 'eu-jecki-lon-blue',
    name: 'Jecki Lon',
    character: 'Jecki Lon',
    era: 'expanded-universe',
    affiliation: 'jedi',
    tier: 'base',
    screenAccurate: true,
    description: 'Jecki Lon\'s blue Padawan blade — a cooler, more analytical shade than her master Sol. Fights with twin-blade precision before her death at Qimir\'s hands.',
    hiltNotes: 'Slim Padawan-issue hilt with a chrome activation slider.',
    config: {
      name: 'JeckiLon',
      baseColor: { r: 40, g: 130, b: 240 },
      clashColor: { r: 220, g: 235, b: 255 },
      lockupColor: { r: 150, g: 200, b: 255 },
      blastColor: { r: 240, g: 250, b: 255 },
      style: 'stable',
      ignition: 'standard',
      retraction: 'standard',
      ignitionMs: 280,
      retractionMs: 340,
      shimmer: 0.07,
      ledCount: 144,
    },
  },

  // ── Vernestra Rwoh (standard blade form, Acolyte) ──
  {
    id: 'eu-vernestra-rwoh-purple-standard',
    name: 'Vernestra Rwoh (Purple - Standard)',
    character: 'Vernestra Rwoh',
    era: 'expanded-universe',
    affiliation: 'jedi',
    tier: 'detailed',
    screenAccurate: true,
    description: 'Master Vernestra Rwoh\'s standard purple lightsaber — the locked-form variant of her signature lightwhip. The youngest Mirialan Jedi Master in centuries, leading the Acolyte investigation with cool detachment.',
    hiltNotes: 'Convertible hilt that locks into a standard blade or unfolds into a lightwhip; this preset is the standard configuration.',
    config: {
      name: 'VernestraStd',
      baseColor: { r: 170, g: 60, b: 240 },
      clashColor: { r: 230, g: 200, b: 255 },
      lockupColor: { r: 200, g: 100, b: 255 },
      blastColor: { r: 220, g: 180, b: 255 },
      style: 'stable',
      ignition: 'standard',
      retraction: 'standard',
      ignitionMs: 300,
      retractionMs: 360,
      shimmer: 0.05,
      ledCount: 144,
      swingFxIntensity: 0.35,
    },
  },

  // ── Master Kelnacca (Acolyte) ──
  {
    id: 'eu-master-kelnacca-blue',
    name: 'Master Kelnacca',
    character: 'Master Kelnacca',
    era: 'expanded-universe',
    affiliation: 'jedi',
    tier: 'base',
    screenAccurate: true,
    description: 'Master Kelnacca — Wookiee Jedi who retreated to a quiet cabin on Khofar before Qimir found him. Deep, steady Jedi blue from one of the few canonical Wookiee Jedi shown on-screen.',
    hiltNotes: 'Oversized hilt sized for a Wookiee grip, carved Mythosaur-spine motif at the pommel.',
    config: {
      name: 'Kelnacca',
      baseColor: { r: 0, g: 100, b: 240 },
      clashColor: { r: 220, g: 230, b: 255 },
      lockupColor: { r: 130, g: 190, b: 255 },
      blastColor: { r: 255, g: 255, b: 255 },
      style: 'stable',
      ignition: 'scroll',
      retraction: 'scroll',
      ignitionMs: 480,
      retractionMs: 440,
      shimmer: 0.05,
      ledCount: 144,
    },
  },

  // ── Torbin (Acolyte) ──
  {
    id: 'eu-torbin-blue',
    name: 'Torbin',
    character: 'Torbin',
    era: 'expanded-universe',
    affiliation: 'jedi',
    tier: 'base',
    screenAccurate: true,
    description: 'Torbin\'s blue blade — the Jedi who took a Barash vow on Olega and meditated for years before Mae forced him out of his silence. A calm, even tone fitting his ascetic discipline.',
    hiltNotes: 'Plain ribbed hilt with no ornamentation — befitting his vow of austerity.',
    config: {
      name: 'Torbin',
      baseColor: { r: 30, g: 130, b: 255 },
      clashColor: { r: 220, g: 240, b: 255 },
      lockupColor: { r: 140, g: 200, b: 255 },
      blastColor: { r: 255, g: 255, b: 255 },
      style: 'stable',
      ignition: 'standard',
      retraction: 'standard',
      ignitionMs: 320,
      retractionMs: 380,
      shimmer: 0.06,
      ledCount: 144,
    },
  },

  // ── Qimir / The Stranger (Acolyte) ──
  {
    id: 'eu-qimir-stranger-red',
    name: 'Qimir / The Stranger',
    character: 'Qimir',
    era: 'expanded-universe',
    affiliation: 'sith',
    tier: 'detailed',
    screenAccurate: true,
    description: 'Qimir — "The Stranger" — wields a deep crimson blade beneath his cortosis-veined helmet. The Sith hunter who manipulates Mae and Osha through The Acolyte; the unstable bleed reflects a master operating outside the Rule of Two.',
    hiltNotes: 'Cortosis-armored hilt with a curved guard and helmet-tethered chain. Designed to deflect lightsaber strikes through its cortosis weave.',
    config: {
      name: 'Qimir',
      baseColor: { r: 180, g: 0, b: 0 },
      clashColor: { r: 255, g: 200, b: 160 },
      lockupColor: { r: 255, g: 70, b: 30 },
      blastColor: { r: 255, g: 100, b: 60 },
      dragColor: { r: 255, g: 90, b: 0 },
      style: 'unstable',
      ignition: 'spark',
      retraction: 'fadeout',
      ignitionMs: 280,
      retractionMs: 340,
      shimmer: 0.18,
      ledCount: 144,
      swingFxIntensity: 0.5,
      noise: 0.12,
    },
  },
];
