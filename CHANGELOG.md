...
- **89sabers V3.9-BT custom-flash feasibility audit** ([`docs/research/PROFFIE_V39BT_FLASH_FEASIBILITY.md`](docs/research/PROFFIE_V39BT_FLASH_FEASIBILITY.md)). 9 custom-firmware flash attempts across 3 bench sessions (2026-05-15, 2026-05-17, 2026-05-18) failed to produce a booting saber; only full dual-bank factory restores work. Forensic analysis of the May 14 backup confirms physical Bank 2 holds factory ProffieOS but physical Bank 1 contains 256 KB of vendor-specific content (no ProffieOS strings — likely a custom 89sabers loader) that the boot chain requires. Custom flash on this chassis is currently **not a sanctioned workflow**; ProffieOS Runtime Presets (`proffie_runtime` board mode, shipped 2026-05-16 in PR #325) is the recommended preset-loading path. Audit covers root-cause hypotheses, recommended protocol, decision matrix, and open experiments.
- **2026-05-18 W2-prime experimental result** ([audit §6](docs/research/PROFFIE_V39BT_FLASH_FEASIBILITY.md)) — compiled 89Sabers' actual factory `89sabers-config.h` source (received via vendor outreach 2026-05-17/18) against our standard `arduino-cli 1.4.1` + `proffieboard:stm32l4 4.6` toolchain, flashed to Bank 1 with `:leave` via `safe-flash.sh`. Produced the same silent-hang failure mode as the 8 prior attempts: chip completes the DFU download, `:leave` submits, transitions to `dfuMANIFEST`, then no USB enumeration of any kind for 30+ seconds. **H4 (toolchain skew at the config level) is largely ruled out; H1 (Bank-1 vendor-loader gate) becomes the leading hypothesis.** USB-only recovery via `restore-factory.sh` was confirmed working end-to-end on the front-side-only chassis (both a rehearsal pre-flash and a real recovery post-failed-flash). After recovery, a 4-preset test deck deployed via direct SD write (`presets.ini` + `presets.tmp` byte-identical) validated the runtime-preset path including **first-hardware confirmation that PR #325's 16-bit RGB encoding fix produces correct visual output on the V3.9-BT** (pure red and pure green via `advanced 65535,0,0…` and `advanced 0,65535,0…` rendered correctly). Bench logs at `scripts/hardware-test/bench-logs/2026-05-18/`; factory-vs-handcrafted config diff at `docs/research/89sabers-config-vs-handcrafted-diff-2026-05-18.txt`.
...
- **R&D next-steps plan** for unblocking V3.9-BT custom flash ([`docs/research/V39BT_FLASH_NEXT_STEPS.md`](docs/research/V39BT_FLASH_NEXT_STEPS.md)) — ST-Link bench session, vendor outreach, stock-Proffieboard reference validation, preset-coverage analysis. Updated 2026-05-18 with W2-prime result.
